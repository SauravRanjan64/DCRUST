...
export async function handleMockRequest(config) {
  ...
  // --- STUDENT ROUTES ---
  if (url === '/api/student/profile' && method === 'GET') {
    const student = students.find(s => s.id === currentSessionUser?.id) || students[0];
    return { status: 200, data: { student } };
  }

  if (url === '/api/student/profile' && method === 'PUT') {
  ...
  if (url === '/api/student/stats' && method === 'GET') {
  ...
  if (url === '/api/resume/upload' && method === 'POST') {
    const studentId = currentSessionUser?.id || 'std-1';
    const newResume = {
  ...
  if (url === '/api/resume' && method === 'DELETE') {
    const studentId = currentSessionUser?.id || 'std-1';
  ...
  if (url === '/api/resume/match' && method === 'POST') {
  ...
  if (url === '/api/company/stats' && method === 'GET') {
  ...
  if (url === '/api/company/drives' && method === 'GET') {
    const compDrives = drives.filter(d => d.companyId === 'comp-1' || d.companyName.includes('ABC'));
    return { status: 200, data: { drives: compDrives } };
  }

  if (url === '/api/company/applicants' && method === 'GET') {
    const compDrives = drives.filter(d => d.companyId === 'comp-1' || d.companyName.includes('ABC'));
    const compDriveIds = compDrives.map(d => d.id);
  ...
  if (shortlistMatch && method === 'POST') {
    ...
    broadcastMockSocket('application:status_updated', {
  ...
  if (url === '/api/company/profile' && method === 'PUT') {
